...
/* $Source: /cs/phd/kobics/.CVSROOT/code/multiClass/mcsvm-ol-train.c,v $ */
/* $Name:  $ */
/* $Locker:  $ */
/* $Revision: 1.10.2.1 $ */
/* $State: Exp $ */
...
static MCDataDef    mc_datadef_svm;
static SPOCParamDef spoc_pd;
static MIRAParamDef mira_pd;
static MCSolution   mc_ol_solution; 
...
int main(int argc, char **argv) {
  ...
  printf("\nMultiClass SVM Train (with online example selection)\tversion 1.0\n"); fflush(stdout);
...
#endif
  mc_ol_solution = mira(mc_datadef, mira_pd);

  printf("Converting OnLine to Batch ... "); fflush(stdout); 
  mc_datadef_svm.m = mc_ol_solution.n_supp_pattern;
  mc_datadef_svm.k = mc_datadef.k; 
  mc_datadef_svm.l = mc_datadef.l;
  mc_datadef_svm.x = (MCDataUnit**) ut_calloc(mc_datadef_svm.m, sizeof(MCDataUnit*));
  mc_datadef_svm.y = (long *) ut_calloc(mc_datadef_svm.m, sizeof(long));
  for (j=0; j<mc_datadef_svm.m; j++) {
    mc_datadef_svm.x[j] = mc_datadef.x[mc_ol_solution.supp_pattern_list[j]]; 
    mc_datadef_svm.y[j] = mc_datadef.y[mc_ol_solution.supp_pattern_list[j]]; 
  }
  printf("done\n"); fflush(stdout);
  mc_cls.solution= spoc(mc_datadef_svm, spoc_pd);
  mc_cls.solution.size = mc_datadef.m;
  for (j=0; j<mc_cls.solution.n_supp_pattern; j++) {
    mc_cls.solution.supp_pattern_list[j] = mc_ol_solution.supp_pattern_list[mc_cls.solution.supp_pattern_list[j]];
  }
  free(mc_datadef_svm.x);
  free(mc_datadef_svm.y);
  ...
  mc_solution_destruct(&mc_ol_solution);
  spoc_destruct();
...
long read_parameters_from_shell(long argc, char **argv) {
  ...
  for (argc--, argv++; argc >0 ; argc-=2, argv++) {
    ...
    case 'a':
      spoc_pd.kernel_def.polynom_a0 = atof(*argv);                                  break;
    case 's':
...
long read_parameters_from_file(char *file_name) {
  ...
  while (fgets(line, STRLEN, file) != NULL) {
	...
	spoc_pd.kernel_def.polynom_a0 = atof(line);
...
void initizlize_mira() {
  mira_pd.beta = spoc_pd.beta;
  mira_pd.epochs =1;
  mira_pd.is_voted =0;

  mira_pd.kernel_def.kernel_type    = spoc_pd.kernel_def.kernel_type;
  mira_pd.kernel_def.polynom_degree = spoc_pd.kernel_def.polynom_degree;
  mira_pd.kernel_def.polynom_a0     = spoc_pd.kernel_def.polynom_a0;
  mira_pd.kernel_def.exponent_sigma = spoc_pd.kernel_def.exponent_sigma;

  mira_pd.redopt_type = spoc_pd.redopt_type;
  mira_pd.delta = spoc_pd.delta;
}
