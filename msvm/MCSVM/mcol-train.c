...
/* $Source: /cs/phd/kobics/.CVSROOT/code/multiClass/mcol-train.c,v $ */
/* $Name:  $ */
/* $Locker:  $ */
/* $Revision: 6.8.4.1 $ */
/* $State: Exp $ */
...
int main(int argc, char **argv) {
  ...
  printf("\nMultiClass OnLine Train\tversion 1.0\n"); fflush(stdout);
  ...
  scale_train_data();
  kernel_text(&mira_pd.kernel_def, stdout);
  
  mc_cls.solution = mira(mc_datadef, mira_pd);
  ...
  mc_cls.kernel_def = mira_pd.kernel_def;
...
void initialize() {
  ...
  mira_pd.kernel_def.kernel_type = KERNEL_EXPONENT_NP;
  mira_pd.kernel_def.polynom_degree = 1;
  mira_pd.kernel_def.polynom_a0 = 1;
  mira_pd.kernel_def.exponent_sigma = .5;

  mira_pd.redopt_type =  REDOPT_TYPE_EXACT;
  mira_pd.delta = 1e-4;
...
long read_parameters_from_shell(long argc, char **argv) {
  ...
  for (argc--, argv++; argc >0 ; argc-=2, argv++) {
    ...
    case 'b':
      mira_pd.beta = atof(*argv);                                                     break;
    case 't':
      mira_pd.kernel_def.kernel_type = (enum KernelType) atoi(*argv);                 break;
    case 'd':
      mira_pd.kernel_def.polynom_degree = atoi(*argv);                                break;
    case 'a':
      mira_pd.kernel_def.polynom_a0 = atof(*argv);                                    break;
    case 's':
      mira_pd.kernel_def.exponent_sigma = atof(*argv);                                break;
    case 'w':
      mira_pd.delta = atof(*argv);                                                    break;
    case 'r':
      mira_pd.redopt_type = (enum RedOptType)  atoi(*argv);                           break;
    case 'g':
      mira_pd.epochs = atof(*argv);                                                   break;  
    case 'q':
      mira_pd.is_voted = atoi(*argv);                                                break;
    case 'p':
...
long read_parameters_from_file(char *file_name) {
  ...
  while (fgets(line, STRLEN, file) != NULL) {
	...
	mira_pd.beta = atof(line);
      else if (strcmp(*table, "kernel_type")==0)
	mira_pd.kernel_def.kernel_type = (enum KernelType) atoi(line); 
      else if (strcmp(*table, "polynom_degree")==0)
	mira_pd.kernel_def.polynom_degree = atoi(line);
      else if  (strcmp(*table, "polynom_a0")==0)
	mira_pd.kernel_def.polynom_a0 = atoi(line);
      else if  (strcmp(*table, "exponent_sigma")==0)
	mira_pd.kernel_def.exponent_sigma = atof(line);
      else if (strcmp(*table, "epochs")==0)
	mira_pd.epochs = atof(line);   
      else if (strcmp(*table, "is_voted")==0)
	mira_pd.is_voted = atoi(line);
      else if  (strcmp(*table, "delta")==0)
	mira_pd.delta = atof(line);
      else if  (strcmp(*table, "redopt_type")==0)
	mira_pd.redopt_type = (enum RedOptType)  atoi(line);
...
void dump(FILE *file) {
  ...
  fprintf(file, "Kernel\n");
  fprintf(file, "\tkernel_type :\t%s\n",kernel_get_type_name(mira_pd.kernel_def.kernel_type));
  fprintf(file, "\tpolynom_degree :\t%ld\n",mira_pd.kernel_def.polynom_degree);
  fprintf(file, "\tpolynom_a0 :\t%f\n",mira_pd.kernel_def.polynom_a0);
  fprintf(file, "\texponent_sigma :\t%f\n",mira_pd.kernel_def.exponent_sigma);
}
